    ...
    <section className="relative min-h-[520px]">
      {active ? (
        <div className="relative overflow-hidden rounded-2xl border border-white/10">
          <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${active.img})`, filter: 'grayscale(10%) brightness(0.9)' }} />
          ...
          <div className="relative z-10 p-6 sm:p-10 space-y-6">
            ...
            <MotionTitle as="h2" className="[font-family:var(--font-playfair)]">
              <div style={{ fontSize: 'clamp(1.8rem,3.6vw,2.6rem)' }}>{active.title}</div>
            </MotionTitle>
            <p className="opacity-80 max-w-3xl [font-family:var(--font-inter)]">{active.sub}</p>
            {active.gallery && (
              <div className="grid gap-3 sm:grid-cols-3">
                {active.gallery.map(src => (
                  <div key={src} className="h-28 sm:h-36 rounded-xl border border-white/10 overflow-hidden" style={{ background: `url(${src}) center/cover no-repeat` }} />
                ))}
              </div>
            )}
            <div className="flex items-center gap-3">
              <button className="btn" onClick={() => setActive(null)}>Explore others signatures</button>
              <a className="btn btn-gold" href="/contact">Establish a connection</a>
            </div>
          </div>
        </div>
      ) : (
        <div className="space-y-8">
          <header>
            ...
            <p className="opacity-70 max-w-2xl [font-family:var(--font-inter)]">Three expressions of the same demand : presence, mastery, and absolute discretion.</p>
          </header>
          <div className="grid md:grid-cols-3 gap-4">
            {items.map((it,i)=> (
              <button key={it.key} type="button" onClick={()=>setActive(it)} className="text-left card card-hover relative overflow-hidden p-4">
                <div className="absolute inset-0 opacity-0 transition-opacity" style={{ backgroundImage: `url(${it.img})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
                     onMouseEnter={(e)=>{ (e.currentTarget as HTMLDivElement).style.opacity = '0.15'; }}
                     onMouseLeave={(e)=>{ (e.currentTarget as HTMLDivElement).style.opacity = '0'; }} />
                <div className="relative">
                  <div className="[font-family:var(--font-playfair)] text-[1.05rem]">{it.title}</div>
                  <div className="opacity-75 text-sm mt-1 [font-family:var(--font-inter)]">{it.sub}</div>
                </div>
              </button>
            ))}
          </div>
